<template>
      <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                  <a href="#" @click="back"><i class="fas fa-arrow-left"></i></a>
                    <ul class="list-group">
                        <li v-if="loading" class="list-group-item"><strong>Loading...</strong></li>
                        <li v-else v-for="song in songs" :key="song.id" class="list-group-item"><router-link :to="{path: '/video/' + song.id }">{{song.title}}</router-link><span style="float:right"><button type="button" class="btn btn-sm btn-primary" @click="addSongToPlaylistModal(song.id)">Add To Playlist</button></span></li>
                    </ul>
                </div>
            </div>
        </div>
...
export default {
  ...
  addSongToPlaylist(playlist_id){
    axios.get('/attachSongToPlaylist/'+playlist_id+'/'+this.songId)
          .then(response => {
